...
// Ports the design's floating light/dark switch. custom.js toggled the
// `_dark_wrapper` class on `._layout_main_wrapper`; we do the same from React
...
export default function DarkModeToggle() {
  ...
  function apply(on: boolean) {
    document
      .querySelectorAll('._layout_main_wrapper')
      .forEach((el) => el.classList.toggle('_dark_wrapper', on));
  }

  useEffect(() => {
    const saved = localStorage.getItem('bs_theme') === 'dark';
    setDark(saved);
    apply(saved);
  }, []);
  ...
  return (
    <div className="_layout_mode_swithing_btn">
      <button
        type="button"
        className={`_layout_swithing_btn_link${dark ? ' _dark_active' : ''}`}
        onClick={toggle}
